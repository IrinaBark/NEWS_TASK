...
<div class="wrapper">
	...
	<div class="local-link">
		...
		<div align="right">
			...
			<a href="controller?command=go_to_base_page&local=en"><c:out
					value="${en_button}" /></a> &nbsp;&nbsp; <a
				href="controller?command=go_to_base_page&local=ru"> <c:out
					value="${ru_button}" />
			</a> <br /> <br />
		</div>
...
<%-- 					</c:if> --%>
					...
					<c:if test="${not (requestScope.AuthenticationError eq null)}">
						<font color="red"> 
							<fmt:message bundle="${loc}" key="${sessionScope.AuthenticationError }"
								var="login_error" /> <c:out value="${login_error}" />
						</font>
					</c:if>
